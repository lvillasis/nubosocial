...
import EditProfileModal from "@/pages/components/EditProfileModal";
...
export default function ProfilePage({ session, userInDb, followersCount: initialFollowers, followingCount: initialFollowing }: Props) {
  ...
  const updatePostLocally = (updatedPost: Partial<Post> & { id?: string }) => {
    if (!updatedPost?.id) {
      console.warn("⚠️ updatePostLocally recibió un objeto sin ID:", updatedPost);
      return;
    }
    ...
    setPosts((prev) =>
      prev.map((p) => {
        ...
        return {
          ...p,
          ...updatedPost,
          author: updatedPost.author?.id ? updatedPost.author : p.author,
        } as Post;
      })
    );
  };
